...
`tsconfig.app.json`) et montés via 2 nouveaux kinds de `Block` (`content/types.ts`) :
...
- `{ kind: 'entrainement', ..., widgetTag? }` — extension additive de l'`entrainement` existant :
  ...
  un remplacement du principe "lien vers le générateur" documenté plus haut.
...
- **4e, Chapitre 1 — La fonction du second degré** (`fonction-second-degre`) : migré en intégralité
  ...
  **Complété une 3e fois** : les 4 widgets interactifs de l'artifact d'origine (jusque-là laissés de
  ...
  — voir « Widgets interactifs portés » ci-dessus pour le mécanisme. `parabole-widget` ("Manipule
  toi-même — fais varier a, b et c") inséré avant "Domaine et image" ; `transformations-widget`
  ("Manipule toi-même — fais varier TH, EV/CV et TV") inséré avant la vidéo "Transformations
  graphiques d'une parabole" dans "Étirement, compression et symétrie" ; `gen7-widget` et
  `gen8-widget` embarqués directement dans les cartes `entrainement` gen7/gen8 existantes
  (`widgetTag`), en plus — jamais à la place — du lien vers la version hébergée sur
